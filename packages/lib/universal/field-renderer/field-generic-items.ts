...
const FIELD_INDICATOR_SUFFIXES = ['conditional-indicator', 'validation-group-indicator', 'required-indicator'];
const FONT_AWESOME_TRIANGLE_EXCLAMATION_PATH =
  'M256 32c14.2 0 27.3 7.5 34.5 19.8l216 368c7.3 12.4 7.3 27.7.1 40.1S485.1 480 470.7 480H41.3c-14.4 0-27.7-7.7-35-20.1s-7.2-27.7.1-40.1l216-368C229.7 39.5 242.8 32 256 32zm0 128c-13.3 0-24 10.7-24 24V296c0 13.3 10.7 24 24 24s24-10.7 24-24V184c0-13.3-10.7-24-24-24zm32 224a32 32 0 1 0-64 0 32 32 0 1 0 64 0z';
...
export const upsertRequiredFieldIndicator = (field: FieldToRender, options: RenderFieldElementOptions) => {
  const existingIndicator = options.pageLayer.findOne(`#${field.renderId}-required-indicator`);
  const indicator =
    existingIndicator instanceof Konva.Group
      ? existingIndicator
      : new Konva.Group({
          id: `${field.renderId}-required-indicator`,
          name: 'required-field-indicator',
          listening: false,
        });
  const color = options.color ? getRecipientColorStyles(options.color).baseRing : '#2563eb';

  if (!(existingIndicator instanceof Konva.Group)) {
    indicator.add(
      new Konva.Path({
        name: 'required-indicator-icon',
        data: FONT_AWESOME_TRIANGLE_EXCLAMATION_PATH,
        scaleX: 16 / 512,
        scaleY: 16 / 512,
        listening: false,
      }),
  ...
  indicator.find('.required-indicator-icon').forEach((icon) => {
    icon.setAttrs({
      fill: color,
    });
  });
